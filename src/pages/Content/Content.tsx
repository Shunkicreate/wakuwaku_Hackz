...
const Content = (): JSX.Element => {
  ...
  const [testUser, setTestUser] = useState<Post | null>()
  ...
  // let testUser: Post
  // fetch("https://wakuwaku-backend.azurewebsites.net/get-post").then((res) => {
  fetch("https://wakuwaku-backend.azurewebsites.net/get-post").then((response) => response.json())
    .then((data) => {
      console.log('Success:', data);
      const sqlData = data as sqlDataType
      const idx = sqlData.data.data.length
      const showData = sqlData.data.data[Math.floor(Math.random() * idx)]
      const preTestUser = {
        post_id: showData.post_id,
        img_url: data(index),
        ...
        happiness_rate: showData.happiness_rate,
        deleted: true
      }
      setDateTime(new Date(preTestUser.posted_at * 1000))
      // let dateTime = new Date(preTestUser.posted_at === null ? 0 : preTestUser.posted_at * 1000);
      setTestUser(preTestUser)
    })
    ...
    <div style={{
      ...
      <div
        ...
        <text style={{ marginLeft: "1vw" }}>
          {dateTime ? dateTime.toLocaleDateString() + " " + dateTime.toLocaleTimeString('ja-JP') : ""}
        </text>
      </div>
